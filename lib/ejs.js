...
function handleCache(options, template) {
  ...
  if (options.cache) {
    ...
    if (!hasTemplate) {
      template = '<!-- ' + path + ' start -->' + fs.readFileSync(path).toString().replace(_BOM, '') + '<!-- ' + path + ' end -->';
    }
  }
  else if (!hasTemplate) {
    ...
    template =  '<!-- ' + path + ' start -->' + fs.readFileSync(path).toString().replace(_BOM, '');
  }
...
function includeSource(path, options) {
  ...
  template =  '<!-- ' + includePath + ' start -->' + fs.readFileSync(includePath).toString().replace(_BOM, '');
...
if (require.extensions) {
  require.extensions['.ejs'] = function (module, filename) {
    ...
    var options = {
      ...
      , template =  '<!-- ' + filename + ' start -->' + fs.readFileSync(filename).toString()
      , fn = exports.compile(template, options);
